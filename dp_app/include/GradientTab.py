from include.UIs.GradientTab_ui import Ui_GradientTab
...
class GradientTab(QWidget, Ui_GradientTab):
    def __init__(self):
        super(GradientTab, self).__init__()
        self.setupUi(self)
    ...
    def readConfig(self):
        ...
        # Read the point_sequence from the INI config file
    ...
    def loadData(self, dataFrame):
        ...
        gradList = ["" for _ in range(len(self.pointSeqs))]
        self.timeListIdx = 0
        self.gradListIdx = 2
        ...
        self.dfTable = pd.DataFrame(
            list(zip(timeList, self.pointSeqs, gradList, self.seqsNotes)), columns=self.tableHeaders
        )
        ...
        # Y-Axes
        self.yNames = [self.realPower3ph, self.reactivePower3ph]
                ...
                if self.testActivated and 0 < index < len(self.xData):
                    ...
                    if self.clickCount < self.ptsPerSeq:
                        ...
                        self.labelGradientVal.setText(
                            f"{self.getGradient(self.clickDataX, self.clickDataY)/self.realPower3phNominal*100*60:.2f}"
                            + " %Pn/min"
                        )
                        ...
                        self.dfTable.iloc[self.seqCount, self.gradListIdx] = (
                            f"{self.getGradient(self.clickDataX, self.clickDataY)/self.realPower3phNominal*100*60:.2f}"
                        )
                        ...
                        self.tableModel.dataChanged.emit(
                            ...
                            self.tableModel.index(self.seqCount, self.gradListIdx),
                        )
