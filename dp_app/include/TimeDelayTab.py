from include.UIs.TimeDelayTab_ui import Ui_TimeDelayTab
...
class TimeDelayTab(QWidget, Ui_TimeDelayTab):
    def __init__(self):
        super(TimeDelayTab, self).__init__()
        self.setupUi(self)
        ...
        self.plots = [self.plot0, self.plot1, self.plot2, self.plot3, self.plot4, self.plot5]

        # Get ViewBoxes from plots
        self.pViewBoxes = [plot.getViewBox() for plot in self.plots]  # type: ignore
        ...
        # Connect sigMouseClicked signals to a onGraphClick method
        ...
        self.btnCancelLastSeq.clicked.connect(self.cancelLastSequence)
    ...
    def readConfig(self):
        ...
        # Read the signalization from the INI config file
        self.signalization = ft.iniReadSectionKey(
            path.join(appBaseDir, "appConfig.ini"),
            "app.time_delay",
            "signalization",
        )

        # Read the commanding from the INI config file
        self.commanding = ft.iniReadSectionKey(
            path.join(appBaseDir, "appConfig.ini"),
            "app.time_delay",
            "commanding",
        )

        # Read the button from the INI config file
        self.button = ft.iniReadSectionKey(
            path.join(appBaseDir, "appConfig.ini"),
            "app.time_delay",
            "button",
        )
        ...
        # Read the point_sequences from the INI config file
        ...
        # Read the sequentions_notes from the INI config file
    ...
    def formatPlots(self):
        ...
        self.xLabelStyle = {"font": "Times", "font-size": "10pt"}
        self.yLabelStyles = [{"color": color, "font": "Times", "font-size": "10pt"} for color in self.plotColors]
        ...
        # Define and add cursor label to the first plot
    ...
    def loadData(self, dataFrame):
        ...
        regDelayList = ["" for _ in range(len(self.pointSeqs))]
        reachTimeList = ["" for _ in range(len(self.pointSeqs))]
        self.timeListIdx = 0
        self.regDelayListIdx = 2
        self.reachTimeListIdx = 3
        ...
        self.dfTable = pd.DataFrame(
            list(zip(timeList, self.pointSeqs, regDelayList, reachTimeList, self.seqsNotes)), columns=self.tableHeaders
        )
        ...
        self.yData = [
            ...
            self.movAvgConvolve(self.df[self.signalization].tolist(), self.movAvgWinSize),
            self.df[self.commanding].tolist()[halfWin + sizeCorrection : -halfWin + 1],
            self.df[self.button].tolist()[halfWin + sizeCorrection : -halfWin + 1],
        ]
    ...
    def mouseMoved(self, event):
        ...
        # DEBUG
        # print(pos)
        # self.labelPos.setText(f"x={pos.x():0.2f}, y={pos.y():0.2f}")
    ...
    def onGraphClick(self, event):
        ...
        if self.plots[j].sceneBoundingRect().contains(event._scenePos):
            mousePoint = self.pViewBoxes[j].mapSceneToView(event._scenePos)  # type: ignore
            ...
            if hasattr(self, "dfTable"):
                ...
                if self.testActivated and 0 < index < len(self.xData):
                    ...
                    if self.clickCount < self.ptsPerSeq:
                        ...
                        # Process Data
                        self.labelTimeDelayVal.setText(f"{self.getTimeDelay(self.clickDataX)/60:.3f} min")
                        ...
                        self.dfTable.iloc[self.seqCount, self.regDelayListIdx] = (
                            f"{self.getTimeDelay(self.clickDataX)/60:.3f}"
                        )

                        self.dfTable.iloc[self.seqCount, self.reachTimeListIdx] = (
                            f"{self.getRiseTime(self.clickDataX)/60:.3f}"
                        )
                        ...
                        self.tableModel.dataChanged.emit(
                            ...
                            self.tableModel.index(self.seqCount, self.reachTimeListIdx),
                        )
                        ...
                        # Append lists to 2D list -> [[x0, x1, ...], [y0, y1, ...], [spot0, spot1, ...]]
